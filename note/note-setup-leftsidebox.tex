...
% huanengchen@foxmail.com 2025-11-29
%===================================
...
\usepackage{tabularray} % 跨页表格添加脚注，参考：https://tex.stackexchange.com/questions/754588/how-can-i-make-the-following-threeparttable-with-footnote-span-multiple-pages-in
\UseTblrLibrary{booktabs} % 跨页表格添加脚注
...
\usepackage{pdfpages} % 在 LaTeX 中插入外部 PDF 文件
...
\DTMsetdatestyle{iso} % YYYY-MM-DD 格式
...
% 定义命令用于添加期刊名称、论文作者、论文发表日期以及论文DOI
\newcommand{\setjournal}[1]{\gdef\journalname{#1}}
\newcommand{\setpaperauthor}[1]{\gdef\paperauthor{#1}}
\newcommand{\setpaperdate}[1]{\gdef\paperdate{#1}}
\newcommand{\setpaperdoi}[1]{\gdef\paperdoi{#1}}
\gdef\journalname{}
\gdef\paperauthor{}
\gdef\paperdate{}
\gdef\paperdoi{}
...
% 定义命令用于添加笔记作者的邮箱
\newcommand{\setauthoremail}[1]{\gdef\authoremail{#1}}
\gdef\authoremail{}

% 给DOI和笔记作者的邮箱添加超链接
\renewcommand{\setpaperdoi}[1]{\gdef\paperdoi{\href{https://doi.org/#1}{#1}}}
\renewcommand{\setauthoremail}[1]{\gdef\authoremail{\href{mailto:#1}{#1}}}
...
\renewcommand{\maketitle}{
    ...
    \end{center}
    {% 添加期刊名称和论文作者
        \ifx\paperauthor\empty
        \else
            \textbf{Paper Author:} \paperauthor \par
            \vspace{-0.2cm}
        \fi
        \ifx\journalname\empty
        \else
    ...
    {% 添加论文发表日期和DOI行
        \ifx\paperdate\empty
            \ifx\paperdoi\empty
                % 两者都为空，不添加额外垂直间距
            \else
                \hfill \textbf{DOI:} \paperdoi \par
            \fi
        \else
            \ifx\paperdoi\empty
                \textbf{Published:} \paperdate \hfill \mbox{} \par
            \else
                \textbf{Published:} \paperdate \hspace{\fill} \textbf{DOI:} \paperdoi \par
            \fi
        \fi
        \vspace{-0.2cm}
    }
    {\textbf{Author:} \@author \hspace{\fill} \textbf{Email:} \authoremail \hspace{\fill} \textbf{Date:} \@date \vspace{-0.2cm}}
...
% 自定义页眉
\renewcommand{\headrulewidth}{0pt} % 设置页眉横线宽度为0pt，消除页眉的横线
...
\usepackage{annotate-equations} % 使用方法参考：https://github.com/st--/annotate-equations

% 统一高亮框高度（解决不同符号高度不一致问题）
% \renewcommand{\eqnhighlightheight}{\vphantom{|\mathcal{I}^*|}\mathstrut} % 使用最高元素作为基准

% 增加注释区域垂直空间
\newcommand{\annotatevspace}[1]{\par\vspace{#1}\noindent} % 创建专用间距命令

% 优化注释样式（减小占用空间）
\tikzset{
    annotate equations/arrow/.style={thin}, % 箭头变细
    annotate equations/text/.style={font=\footnotesize} % 注释文字缩小
}
