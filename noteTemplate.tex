...
\renewcommand{\chaptermark}[1]{\markboth{\bfseries#1}{}} % 在双面打印时将章节标题（#1）标记在偶数页的页眉（左边），而奇数页的页眉保持为空。
\renewcommand{\sectionmark}[1]{\markright{\thesection\ \bfseries#1}} % 在单面或双面打印时都将小节编号和标题（\thesection\ #1）标记在奇数页的页脚（右边）。
\fancyhf{} % 清空原本的页脚页眉样式
\fancyhead[RO]{\thepage} % 设置奇数页（[RO]，即Right, Odd）的页眉右侧（内边）显示的内容为页码（\thepage）。
\fancyhead[LO]{
    % The \S only if the section number is greater than 0.
